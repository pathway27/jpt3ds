...
#include "Grid.h"
#include "GridHiragana.h"

using namespace jlpt;

GridHiragana::GridHiragana(Component* parent, char* imageName,
  char* imageKeyName) :
  Grid(parent, imageName, imageKeyName) {
  keyboardMode = KB_NORMAL;
}

int GridHiragana::dealEvent(SCREEN scr) {
  if (visible && screen == scr) {
    int level = scoreManager->getLevel(0);
...
void GridHiragana::displayIdeogram(int i, int j) {
  ...
  // Affichage du caractere selectionne
    ...
    PA_LoadSpritePal(SCREEN_UP, PALETTE_0,
        (void*)hiragana_Pal[i][j]);
    PA_CreateSprite(SCREEN_UP, SPRITE_0,
        (void*)hiragana_Sprite[i][j], OBJ_SIZE_64X64,COLOR_256, PALETTE_0, 21 , 53);
...
void GridHiragana::display(SCREEN scr) {
  ...
  if (visible && screen == scr) {
    ...
    if (imgTiles == NULL) {
      ...
      int x = getX();
      int y = getY();
      int level = scoreManager->getSelectedRecord().level[0];
      ...
      if (level >= 10) {
        ...
        if (keyboardMode == KB_DAKUTEN) {
          drawAnimation_1(x0, y0, x1, y1, RGB15(0, 31, 0));
        }
